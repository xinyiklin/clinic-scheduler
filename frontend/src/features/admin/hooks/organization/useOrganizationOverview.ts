...
function getCurrentOrganizationId(user: UserProfile | null) {
...
export default function useOrganizationOverview() {
  ...
  const overviewQuery = useQuery({
    queryKey,
    queryFn: () => (organizationId ? fetchOrganization(organizationId) : null),
    enabled: Boolean(organizationId),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, values }: UpdateOrganizationPayload) =>
