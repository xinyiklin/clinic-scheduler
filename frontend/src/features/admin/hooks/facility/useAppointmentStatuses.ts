...
type SaveAppointmentStatusPayload = {
  id?: EntityId | null;
  values: ApiPayload;
};

function getAppointmentStatusesQueryKey(
...
export default function useAppointmentStatuses(
  facilityId: EntityId | null | undefined
) {
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ id, values }: SaveAppointmentStatusPayload) => {
  ...
  const saveStatus = async ({ id, values }: SaveAppointmentStatusPayload) => {
    if (!facilityId) return;
    return saveMutation.mutateAsync({ id, values });
  };

  const removeStatus = async (id: EntityId) => {
    if (!facilityId) return;
