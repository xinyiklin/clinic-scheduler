...
import type { ScheduleMode } from "./useSchedulePageResources";

type UseSchedulePageColumnsOptions = {
  facility?: FacilityLike | null;
  preferences: UserPreferences;
  resources: ResourceLike[];
};

function sanitizeScheduleStartMode(value: unknown): ScheduleMode {
...
export default function useSchedulePageColumns({
  ...
  const handleScheduleIntervalChange = useCallback(
    (nextInterval: number | string) => {
      const normalizedInterval = SLOT_INTERVAL_OPTIONS.includes(
        Number(nextInterval) as (typeof SLOT_INTERVAL_OPTIONS)[number]
      )
        ? Number(nextInterval)
        : DEFAULT_SLOT_INTERVAL;
      const nextLength = Math.min(
  ...
  const handleSelectScheduleDate = useCallback((nextDate: string) => {
    if (!nextDate) return;
